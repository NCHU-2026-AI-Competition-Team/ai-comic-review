...
确保 storage/audio/{video_id}/audio.wav 存在（缺失时自动从上传视频提取），
...
空文本分段不产生事件；音频提取失败（无音轨、上传文件缺失）明确抛错。
"""
...
def segments_to_events(
    ...
    时间戳直接取分段的 start_ms/end_ms（int 毫秒），保证多模态事件
    可基于同一时间轴融合；置信度截断到 [0, 1]，避免引擎异常值导致
    事件校验失败。
    """
    events = []
    for index, segment in enumerate(segments):
        ...
        events.append(
            TimelineEvent(
                ...
                confidence=_clamp_confidence(segment.confidence, index),
                metadata={"language": language, "segment_index": index},
            )
        )
    return events
...
def asr_result_path(video_id: str) -> Path:
    ...
    return get_settings().outputs_path / video_id / ASR_RESULT_FILENAME


def run_asr(video_id: str, provider: Optional[AsrProvider] = None) -> list[TimelineEvent]:
    """对指定视频执行 ASR，聚合结果落盘 asr.json 并返回事件列表。

    audio.wav 不存在时自动从上传视频提取（无音轨抛 NoAudioTrackError、
    ...
    result_file = asr_result_path(video_id)
